sap.ui.core.mvc.Controller.extend("sap.ui.demo.tdg.view.AddProduct", {
        ...
        onSave : function() {
        	    // Initial errorModel
	            if(!this.getView().getModel("errModel")){
	    	    	this.getView().setModel(new sap.ui.model.json.JSONModel(),"errModel");
	    	    }
                ...
                if (!this.getView().getModel("newProduct").getProperty("/Detail/Name")) {
                            ...
                            this.oAlertDialog = sap.ui.xmlfragment("sap.ui.demo.tdg.view.ErrorDialog", this);
                    	    this.getView().getModel("errModel").setProperty("/errMessage",
                    	    		this.getView().getModel("i18n").getProperty("nameRequiredDialogText"));
                            this.getView().addDependent(this.oAlertDialog);
                        this.oAlertDialog.open();
                        return;
                }
                if(this.getView().getModel("newProduct").getProperty("/Detail/ReleaseDate")){
                	var realseday = new Date(this.getView().getModel("newProduct").getProperty("/Detail/ReleaseDate"));
                	var today = new Date();
                	if(realseday.dateValue > today){
                        this.oAlertDialog = sap.ui.xmlfragment("sap.ui.demo.tdg.view.ErrorDialog", this);
                	    this.getView().getModel("errModel").setProperty("/errMessage",
            	    			this.getView().getModel("i18n").getProperty("realsedateLate"));
                        this.getView().addDependent(this.oAlertDialog);
            	    	this.oAlertDialog.open();
                        return;
                	};
                }
                if(this.getView().getModel("newProduct").getProperty("/Detail/Price")){
                	if(this.getView().getModel("newProduct").getProperty("/Detail/Price") > 99999.99){
                        this.oAlertDialog = sap.ui.xmlfragment("sap.ui.demo.tdg.view.ErrorDialog", this);
                	    this.getView().getModel("errModel").setProperty("/errMessage",
            	    			this.getView().getModel("i18n").getProperty("priceOver"));
                        this.getView().addDependent(this.oAlertDialog);
            	    	this.oAlertDialog.open();
                        return;
                	};
